...
const index = (props) => {
  const { open, toggle, form, setForm, postCategory, setUpdate } = props;
  const initialValues = {
    name: form.name || "",
  };
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} className="w-[450px] p-3 bg-white rounded-xl">
        ...
        <Formik initialValues={initialValues} onSubmit={handleSubmit} validationSchema={categoryValidationSchema}>
          <Form id="form">
            <Field name="name" as={TextField} type="text" margin="normal" variant="outlined" fullWidth helperText={<ErrorMessage name="name" component="p" className="text-[red] text-[15px]" />} label="Name" />
          </Form>
        </Formik>
        <div>

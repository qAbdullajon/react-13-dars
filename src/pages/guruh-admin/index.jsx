...
const index = () => {
  ...
  const handleDelet = (id) => {
    axios.delete(`http://localhost:3000/guruh/${id}`).then((res) => {
      if (res.status === 200) {
        getGuruh();
      }
    });
  ...
  const postTeacher = (user) => {
    if (update) {
      axios.put(`http://localhost:3000/guruh/${form.id}`, user).then((res) => {
        if (res.status === 200) {
          getGuruh();
          setOpen();
        }
      ...
      axios.post(`http://localhost:3000/guruh/`, user).then((res) => {
        if (res.status === 201) {
          getGuruh();
          setOpen(false);
        }
  ...
  return (
    <div>
      <GuruhModal open={open} toggle={setOpen} postTeacher={postTeacher} form={form} setForm={setForm} setUpdate={setUpdate} course={course} />
      <div className="flex justify-between items-center">
        <h1 className="text-3xl">Guruh</h1>
